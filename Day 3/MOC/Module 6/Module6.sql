...
*/
--Local Variable 
--Declare <Name> <Type>   = <Intital Value>
Declare @name nvarchar(10) ='Sayed Said Hawas'
Select @name 

Declare @x nvarchar ='Welcome in SQL Serve'
Select @x

Declare @y nvarchar(50) =N'???? ??? ?? ?????'
Select @y

Declare @y varchar(50) =N'???? ??? ?? ?????'
Select @y

--built-in function 
Declare @y char(8000) ='Welcome in SQL Server ';
Select len(@y) as AutLen , Datalength(@y) As DatatypeLen,@y

declare @n varchar(100) ='Welcome in SQL Server ';
Select len(@n) as AutLen , Datalength(@n) As DatatypeLen,@n

declare @n varchar(8000) ='Welcome in SQL Server';
Select len(@n) as AutLen , Datalength(@n) As DatatypeLen,@n

declare @n varchar(MAX) ='Welcome in SQL Server';
Select len(@n) as AutLen , Datalength(@n) As DatatypeLen,@n


declare @n  tinyint = 256
Select @n


declare @Salary  decimal(9,2) = 1234568.79
Select @Salary
