...
Select e.Name,e.Job,e.salary ,d.Name As [Department Name] 
...
Select isNULL(emp.FirstName,'Chairman') as Manager  , Mag.FirstName as employee
from Employees as emp right Join Employees as Mag 
ON Mag.ReportsTo=emp.EmployeeID
...
Select * from Vw_ShowOrder

SELECT [Order Details].ProductID, [Order Details].OrderID, [Order Details].Quantity AS QTY, [Order Details].UnitPrice, Orders.CustomerID, Orders.OrderDate, Employees.LastName, Employees.FirstName
FROM     Employees INNER JOIN
                  Orders ON Employees.EmployeeID = Orders.EmployeeID FULL OUTER JOIN
                  [Order Details] ON Orders.OrderID = [Order Details].OrderID
WHERE  ([Order Details].UnitPrice > 200)
